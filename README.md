...
### Reranker 模型显示"未下载"怎么办？

轻量 reranker 模型（ms-marco-MiniLM-L6-v2, ~23MB）和标准 reranker 模型（bge-reranker-base, ~279MB）**不随仓库附带**（onnx 文件被 .gitignore 排除，体积太大）。

模型不随仓库附带，缺失时自动关闭 reranker，不影响基础聊天和基础 RAG。

如果你想启用 reranker，后续版本会提供"下载模型"按钮（支持配置镜像源）。当前阶段可以暂时使用"关闭"模式。
