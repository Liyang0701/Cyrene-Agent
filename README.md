...
**一句话：这个 Fork 把 Cyrene 的桌面语音能力扩展成了一套可以由 Android
手机在公网安全呼叫、管理历史并继续聊天的个人语音伴侣系统。**

手机不是另起一个云端机器人。角色、记忆、模型、ASR、GPT-SoVITS/TTS 和工具仍
运行在你的 Cyrene 桌面端；Android 负责远程呼叫、麦克风、扬声器与对话管理：

```text
Android Cyrene Voice
  ├─ HTTPS：配对、呼叫、权威状态 → Cloudflare Worker + Durable Object
  └─ WebRTC + 媒体 E2EE ──────────→ LiveKit Room
                                           ↕
                                  你的 Cyrene 桌面端
                          ASR → 角色/记忆/模型 → TTS
```
...
| 嘈杂环境输入 | 无 | 自动聆听与手动轮次双模式，旁人较多时可按轮次开放 ASR |
| 移动端音质与路由 | 无 | 48 kHz 高质量下行，蓝牙、扬声器、耳机和听筒切换 |
| 通话可靠性 | 无完整跨端状态机 | 幂等呼叫、E2EE 就绪门禁、重连、残留通话替换和终态收敛 |
| 部署资料 | 无这套移动链路说明 | 架构、安全、EAS APK、从零部署、验收矩阵与 FAQ |

### 它解决什么问题？

- **离开电脑也能与同一个 Cyrene 交流**：继续使用桌面端已经配置好的角色、
  记忆、模型和语音，不需要重新做一套云端 Agent。
- **保留本地能力与隐私边界**：Cloudflare 只协调授权与通话状态，LiveKit 只承载
  加密媒体；模型密钥、对话正文和 GPT-SoVITS 不迁入控制面。
- **对个人长期使用更完整**：手机只需首次配对；之后可以覆盖升级、选择旧对话
  继续、管理历史，并在安静或嘈杂环境间切换输入方式。
- **便于别人复现而不是只能运行作者环境**：公开版本移除了维护者的 Worker、
  EAS Project、签名和生产凭据，使用者按文档创建自己的基础设施。
