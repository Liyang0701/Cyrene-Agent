...
import "./asr/mobile-call-panel";  // 手机设备授权、长期配对与 Beta LiveKit 通话
...
import { loadCharacterPackages } from "./character/panel";
...
if (!window.settings) {
  (window as unknown as { settings: SettingsApi }).settings = {
    ...
    listCharacters: async () => ({ status: "failed", activeCharacter: null, packages: [] }),
    pickCharacterImportFolder: async () => null,
    importCharacter: async () => ({ ok: false, status: "failed", diagnostics: [{ code: "unavailable", message: "settings api unavailable" }] }),
    switchCharacter: async () => ({ ok: false, status: "failed", diagnostics: [{ code: "unavailable", message: "settings api unavailable" }] }),
    uninstallCharacter: async () => ({ ok: false, diagnostics: [{ code: "unavailable", message: "settings api unavailable" }] }),
    listArchivedCharacterStates: async () => [],
    deleteArchivedCharacterState: async () => ({ ok: false, diagnostics: [{ code: "unavailable", message: "settings api unavailable" }] }),
    openCustomStylePrompt: async () => ({ ok: false, error: "settings api unavailable" }),
    channelsGetConfig: () => Promise.resolve({ wechat: {}, feishu: {} }),
...
const NAV_LABELS: Record<string, { emoji: string; title: string; hint: string }> = {
  ...
  cyrene: { emoji: "🌸", title: "角色设置", hint: "管理 Agent 行为、记忆、RAG 与权限" },
...
function switchSection(section: string): void {
  ...
  if (charactersPanel) charactersPanel.classList.toggle("is-hidden", !isCharacters);
  ...
  placeholderPanel.classList.toggle(
    "is-hidden",
    isApi || isApiAdvanced || isAppearance || isGeneral || isPreferences || isCyrene || isCharacters || isDisclaimer || isMemory || isUser || isTasks || isPlugins || isSkills || isTokens || isChannels || isTts || isAsr || isMusic,
  );
