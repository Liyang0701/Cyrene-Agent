  ...
  private loadState(): void {
    ...
    try {
      ...
      for (const [id, value] of Object.entries(record.entries as Record<string, unknown>)) {
        if (!this.dmae.getState(id) || !value || typeof value !== "object") continue;
        ...
        const recentUserHits = Array.isArray(candidate.recentUserHits)
          ? candidate.recentUserHits.filter((turn): turn is number => Number.isFinite(turn)).map(Math.trunc)
          : [];
        this.dmae.setState(id, {
          activation: Math.max(0, Math.min(maxScore, candidate.activation as number)),
  ...
  private saveState(): void {
    ...
    try {
      ...
      const entries = Object.fromEntries(
        this.entries
          .map((entry) => [entry.id, this.dmae.getState(entry.id)] as const)
          .filter((entry): entry is readonly [string, EntryState] => entry[1] !== undefined)
          .map(([id, state]) => [id, { ...state, recentUserHits: [...state.recentUserHits] }]),
      );
