...
describe("exportMemoryToObsidianVault", () => {
  ...
  beforeEach(async () => {
    ...
    state.configureActiveCharacterState(
      state.resolveCharacterStateLayout(electronMock.userDataDir, "test"),
    )
  })
  ...
  it("generates [[wikilinks]] for L2 <-> entity relations (bidirectional)", async () => {
    ...
    writeFile(
      path.join(electronMock.userDataDir, "characters", "test", "memory"),
      "entity-graph.json",
      JSON.stringify(entityGraphData, null, 2),
    )
