...
import { isCallActive, registerCallIpc, setCallSettings, stopCall } from "./call/call-manager";
...
import { registerCharacterIpc } from "./character/character-ipc";
import {
...
import { configureActiveCharacter, getActiveCharacter } from "./character/active-character";
...
import {
  ...
  type RemoteAccessSubsystem,
} from "./remote-access/bootstrap";
...
let remoteAccessSubsystem: RemoteAccessSubsystem | null = null;
...
registerCharacterIpc({
  ...
  coordinateSwitch: (operation) => channelsSubsystem?.coordinateCharacterSwitch(operation) ?? operation(),
});
...
app.whenReady().then(async () => {
  characterRuntime = createDefaultCharacterRuntime({
    ...
    switchAdapters: createElectronCharacterSwitchAdapters({
      getActivity: () => {
        ...
        return {
          agentBusy: hasUncoordinatedAgentActivity(
            proactiveLifecycle.isConversationBusy() ? 1 : 0,
            channelsSubsystem?.getWechatProcessingCount() ?? 0,
          ),
          callActive: isCallActive(),
          asrBusy: localAsrWorker.isBusy(),
          ttsBusy: activity.tts,
          proactiveBusy: proactiveLifecycle.getProactiveChatService()?.isGenerating() ?? false,
      ...
      stopScheduler: () => schedulerSubsystem?.engine.stop(),
      stopOpener: () => {},
      shutdownChannels: () => channelsSubsystem?.shutdown(),
  ...
  bootstrapPermission();
  registerCallIpc();
  remoteAccessSubsystem = bootstrapRemoteAccess();
  try {
...
app.on("before-quit", () => {
  stopCall();
  void remoteAccessSubsystem?.stop();
  disposeAsr();
