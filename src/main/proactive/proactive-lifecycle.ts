...
import { getActiveCharacter, getActiveCharacterText } from "../character/active-character";
...
export function createProactiveLifecycle(options: ProactiveLifecycleOptions): ProactiveLifecycle {
  ...
  function buildProactivePersonaPrompt(): string {
    ...
    return composeCharacterSystemPrompt({
      applicationPolicy: [loadPromptFile("application_policy.md"), loadPromptFile("chat_system.md")]
        .filter(Boolean)
        .join("\n\n---\n\n"),
      character: {
        ...character,
  ...
  return {
    ...
    isConversationBusy: () => normalConversationBusyCount > 0,
    proactiveConversationLifecycle,
  };
}
