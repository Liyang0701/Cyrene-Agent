...
export interface AgentRuntimeDeps {
  ...
  loadChannelUserProfile: NonNullable<BuildOptionsDeps["loadChannelUserProfile"]>;
  buildChannelRelationshipContext: NonNullable<BuildOptionsDeps["buildChannelRelationshipContext"]>;
  recordChannelRelationshipTurn: NonNullable<OnRunFinishedDeps["recordChannelRelationshipTurn"]>;
...
export function createAgentRuntime(rawDeps: AgentRuntimeDeps): AgentRuntime {
  ...
  async function observeRuntimeState(
    ...
    await rawDeps.enqueueLLMTask(
      "心情观察器",
      async () => {
        const characterName = getActiveCharacterText().displayName;
          ...
          [
            {
              role: "system",
              content:
                `你是一个情绪分析器。以下是${characterName}的完整人格设定：\n\n` +
                loadSoulFeelingContext() +
                `\n\n根据以上人格设定和以下对话，判断${characterName}当前的心情状态。可选心情值（只能选其中一个）：平静 / 开心 / 温柔 / 激动 / 撒娇 / 担心 / 难过 / 感动 / 害羞。只返回 JSON，不要任何多余文字：{\"feeling\": \"心情值\"}。判断规则：以最后一轮对话为主，之前几轮为辅；判断的是${characterName}的心情，不是用户的心情；无法判断时返回 平静。`,
            },
            {
              role: "user",
  ...
  function buildBuildOptionsDeps(): BuildOptionsDeps {
    return {
      ...
      buildAlwaysOnContext: ((userText, messages, options) =>
        buildAlwaysOnContext(userText, messages as any, options)) as BuildOptionsDeps["buildAlwaysOnContext"],
      buildRelationshipContext,
      loadChannelUserProfile: rawDeps.loadChannelUserProfile,
      buildChannelRelationshipContext: rawDeps.buildChannelRelationshipContext,
      buildSystemPrompt,
  ...
  function buildOnRunFinishedDeps(): OnRunFinishedDeps {
    return {
      ...
      canUseActiveCharacterStickers: rawDeps.canUseActiveCharacterStickers,
      ...
      recordChannelRelationshipTurn: rawDeps.recordChannelRelationshipTurn,
    };
  }
