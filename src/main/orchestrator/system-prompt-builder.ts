...
export function readStylePrompt(styleId: StyleId): string {
  ...
  const activeStylePath = active.stylesDirectoryPath
    ? path.join(active.stylesDirectoryPath, STYLE_FILE_BY_ID[styleId])
    : null;
  if (activeStylePath && fs.existsSync(activeStylePath)) {
    return fs.readFileSync(activeStylePath, "utf8").trim();
  }
  return loadPromptFile("styles/" + STYLE_FILE_BY_ID[styleId]);
}

function generalizeApplicationPrompt(value: string): string {
  return value
    .replaceAll(getActiveCharacterText().displayName, "活动角色")
    .replaceAll("她", "当前角色");
}
